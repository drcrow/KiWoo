<?php
/**
 * Country / State / City Customizations
 */

/**
 * Replace Woo Conutries with the Kiboo Default Contry
 */

add_filter('woocommerce_countries','custom_country', 10, 1);

function custom_country($woo_countries){

    //die('countries '.print_r($woo_countries, true));

    foreach($woo_countries as $countryID => $countryName){
        if(kibooGetOption('woo_api_country') != '' && kibooGetOption('woo_api_country') != $countryID){
            unset($woo_countries[$countryID]);
        }
    }

    //die('countries '.print_r($woo_countries, true));

    return $woo_countries;
}

/**
 * Replace Woo States With Kiboo States
 */
add_filter( 'woocommerce_states', 'custom_woocommerce_states', 10, 1 );

function custom_woocommerce_states( $woo_states ) {

    //die('states '.print_r($woo_states, true));
    ...
    foreach($states as $state){
        $woo_states['AR'][$state->ID] = $state->name;
    }
...
/**
 * City & State fields as Selects with autocomplete
 */

//https://github.com/8manos/wc-city-select
...
class WC_City_Select {
    ...
    public function __construct() {
        add_filter( 'woocommerce_billing_fields', array( $this, 'billing_fields' ), 1, 2 );
        add_filter( 'woocommerce_shipping_fields', array( $this, 'shipping_fields' ), 1, 2 );
        add_filter( 'woocommerce_form_field_city', array( $this, 'form_field_city' ), 1, 4 );
